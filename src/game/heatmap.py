...
class Heatmap:
    ...
    def score(self, zone: str) -> float:
        ...
        """
        with self._lock:
            rounds = dict(self._data.get(zone) or {})
            current_round = self._current_round
        if not rounds:
            return 0.0
        total = 0.0
        for r, v in rounds.items():
            age = max(0, current_round - r)   # 0 = this round; guard future rounds
    ...
    def hottest_zones(self, n: int = 5) -> List[Tuple[str, float]]:
        ...
        with self._lock:
            zone_names = list(self._data.keys())
        scores = [(z, self.score(z)) for z in zone_names]
